...
public class EvaluationSelfEvaluation {

    private Credentials user = new Credentials();

    public void selfEvaluation(Long taskId, int perfomance, String login) throws Exception {
    ...
    private TaskSummary findTask(List<TaskSummary> tasks, long taskId) {
        for (TaskSummary task: tasks) {
            if (task.getId() == taskId && task.getName().equals("Self Evaluation")) {
                return task;
            }
        }
